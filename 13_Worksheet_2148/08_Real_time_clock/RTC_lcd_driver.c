...
/*
* Function Name : void lcd_intNo(int no)
* Description   : This Function used to send integer numbers on lcd
*                 
*/
void lcd_intNo(int no)
{
	// Varrible to store no converted in char 
	char num[15];
	// Varrible for index in string
	int i=0;
	// Check no is zero 
	if(no==0)
	{
		// Function call to display 0 in lcd
		lcd_data('0');
		// function terminate
		return;
	}
	// checking no is less than zero
	if(no<0)
	{
		// Function call to display '-' sign on lcd 
		lcd_data('-');
		// negative no conversion in positive
		no=-no;
	}
	// wait till no is greater than zero
	while(no>0)
	{
		// store no in as a char in string 
		num[i++]=((no%10)+48);
		// delete last digit of no
		no/=10;
	}
	// loop iterate in reverse to display char 
	for(i--;i>=0;i--)
	{
		// function call to display no in reverse order
		lcd_data(num[i]);
	}
}
